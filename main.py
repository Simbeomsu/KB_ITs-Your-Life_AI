...
from detectword import convertCoordination, read_img_by_coord
...
def run(target):
    ...
    for i in target:
        ext = os.path.splitext(i)[-1] #파일의 확장자를 출력하는 함수
        if ext.lower() == '.pdf':
            ...
            img = cv2.imread(i, 0) #이미지파일 읽기
            cv2.imwrite('{}/{}.jpg'.format(img_dir, os.path.basename(i)[:-4]), img) #img폴더에 읽은 이미지를 저장함.
        elif ext.lower() == '.tif':
            img = cv2.imread(i, 0)
    ...
    #detect
    img = glob.glob('./img/*.jpg') #img안에 있는 모든 jpg확장자의 파일을 받음
    #for im in img:
    ...
    len_loc, loc = convertCoordination() #얻은 두개가 뭐야?
...
# target = ['./test/10-20190207162430645.jpg']
# target = ['./test/20201215_152649_d2085b19b1c447b0a9bc15008ae7bf59.jpg']
# data = run(target)
